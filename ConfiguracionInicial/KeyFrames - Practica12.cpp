...
void saveFrame(void)
{
	...
	KeyFrame[FrameIndex].rotDogSit = rotDogSit;
...
void resetElements(void)
{
	...
	rotDogSit = KeyFrame[0].rotDogSit;
...
void interpolation(void)
{
	...
	KeyFrame[playIndex].rotDogSitInc = (KeyFrame[playIndex + 1].rotDogSit - KeyFrame[playIndex].rotDogSit) / i_max_steps;
...
void saveAnimation(const char* filename) {
	...
	for (int i = 0; i < FrameIndex; i++)
	{
		fprintf(file, "%f %f %f %f %f %f %f %f %f %f\n",
			...
			KeyFrame[i].FLegs, KeyFrame[i].RLegs, KeyFrame[i].FRightLeg,
			KeyFrame[i].rotDogSit
		);
	}

	fclose(file);
	printf("Guardado en archivo correctamente\n");
}
...
void loadAnimation(const char* filename) {
	...
	if (err != 0 || !file)
	{
		printf("No existe archivo previo, guarda una nueva animacion\n");
		clearAnimation();
		return;
	}

	fscanf_s(file, "%d", &FrameIndex);

	for (int i = 0; i < FrameIndex; i++)
	{
		fscanf_s(file, "%f %f %f %f %f %f %f %f %f %f",
			&KeyFrame[i].dogPosX, &KeyFrame[i].dogPosY, &KeyFrame[i].dogPosZ,
			&KeyFrame[i].rotDog, &KeyFrame[i].head, &KeyFrame[i].tail,
			&KeyFrame[i].FLegs, &KeyFrame[i].RLegs, &KeyFrame[i].FRightLeg,
			&KeyFrame[i].rotDogSit
...
void clearAnimation() {
	...
	dogPosX = dogPosY = dogPosZ = 0;
	rotDog = head = tail = 0;
	FLegs = RLegs = 0;
	FRightLeg = 0;
	rotDogSit = 0;

	printf("La animacion guardada ha sido borrada\n");
}
...
int main()
{
	...
	printf("1) Cargar animacion\n2) Nueva animacion\nSeleccion: ");
	scanf_s("%d", &menu);

	if (menu == 1)
		loadAnimation("animacion.txt");
	else
		clearAnimation();
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Animacion keyframes - Practica 12 - Cecilia Torres", nullptr, nullptr);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		modelTemp = model = glm::rotate(model, glm::radians(-rotDogSit * 0.5f), glm::vec3(1.0f, 0.0f, 0.0f)); //sit
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		...
		//Front Right Leg Modified
		model = modelTemp;
		model = glm::translate(model, glm::vec3(-0.111f, -0.055f, 0.074f));
		model = glm::rotate(model, glm::radians(FLegs + FRightLeg), glm::vec3(1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		F_RightLeg.Draw(lightingShader);
		//Back Left Leg
		model = modelTemp; 
		model = glm::translate(model, glm::vec3(0.082f, -0.046, -0.218)); 
		model = glm::rotate(model, glm::radians(RLegs - rotDogSit), glm::vec3(1.0f, 0.0f, 0.0f)); 
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model)); 
...
void Animation() {
	...
	if (play)
	{
		...
		else
		{
			...
			rotDogSit += KeyFrame[playIndex].rotDogSitInc;
